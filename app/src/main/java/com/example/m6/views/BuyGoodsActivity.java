...
public class BuyGoodsActivity extends AppCompatActivity implements BuyDialog.BuyDialogListener {
    ...
    public void clickMaxButton(Button button, final int price, final Goods goods) {
        button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(IsAble(goods)) {
                    int max = Math.min(player.getSpaceship().getBay() - player.getCargo(),
                            player.getCredit() / price);
                    if (max > 0) {
                        player.getInven().put(goods.toString().toLowerCase(), player.getInven()
                                .get(goods.toString().toLowerCase())+max);
                        player.setCargo(player.getCargo() + max);
                        player.setCredit(player.getCredit() - max * price);
                        bay.setText(String.valueOf(player.getCargo()) + "/" + player.getSpaceship()
                                .getBay());
                        credit.setText(String.valueOf(player.getCredit()) + " Cr");
    ...
    public void setupStock(){
        ...
        if(IsAble(Goods.WATER)){
            n = String.valueOf(player.getCurrentplanet().getStock().get(Goods.WATER.toString()
                    .toLowerCase()));
//            Log.d("stock", n);
        } else {
            n="N/A";
        ...
        if(IsAble(Goods.FURS)){
            n = String.valueOf(player.getCurrentplanet().getStock().get(Goods.FURS.toString()
                    .toLowerCase()));
        } else {
            n="N/A";
        }
        ...
        if(IsAble(Goods.FOOD)){
            n = String.valueOf(player.getCurrentplanet().getStock().get(Goods.FOOD.toString()
                    .toLowerCase()));
        } else {
            n="N/A";
        }
        ...
        if(IsAble(Goods.ORE)){
            n = String.valueOf(player.getCurrentplanet().getStock().get(Goods.ORE.toString()
                    .toLowerCase()));
        } else {
            n="N/A";
        }
        ...
        if(IsAble(Goods.GAMES)){
            n = String.valueOf(player.getCurrentplanet().getStock().get(Goods.GAMES.toString()
                    .toLowerCase()));
        } else {
            n="N/A";
        }
        ...
        if(IsAble(Goods.FIREARMS)){
            n = String.valueOf(player.getCurrentplanet().getStock().get(Goods.FIREARMS.toString()
                    .toLowerCase()));
        } else {
            n="N/A";
        }
        ...
        if(IsAble(Goods.MEDICINE)){
            n = String.valueOf(player.getCurrentplanet().getStock().get(Goods.MEDICINE.toString()
                    .toLowerCase()));
        } else {
            n="N/A";
        }
        ...
        if(IsAble(Goods.MACHINES)){
            n = String.valueOf(player.getCurrentplanet().getStock().get(Goods.MACHINES.toString()
                    .toLowerCase()));
        } else {
            n="N/A";
        }
        ...
        if(IsAble(Goods.NARCOTICS)){
            n = String.valueOf(player.getCurrentplanet().getStock().get(Goods.NARCOTICS.toString()
                    .toLowerCase()));
        } else {
            n="N/A";
        }
        ...
        if(IsAble(Goods.ROBOTS)){
            n = String.valueOf(player.getCurrentplanet().getStock().get(Goods.ROBOTS.toString()
                    .toLowerCase()));
        } else {
            n="N/A";
        }
    ...
    public void buyItem(String goods, int price) {
        ...
        if (max > 0) {
            if(player.getCredit() - Integer.parseInt(inputStr) * price >= 0 && player.getCargo()
                ...
                Toast.makeText(getApplicationContext(), "You do not have enough room "
                        + "in your cargo.", Toast.LENGTH_LONG).show();
            } else {
                Toast.makeText(getApplicationContext(), "you are trying to buy more than "
                        + "planet has", Toast.LENGTH_LONG).show();
            }
        } else {
            Toast.makeText(getApplicationContext(), "You can not buy anymore. Check your bay"
                    + "or credit.", Toast.LENGTH_LONG).show();
        }
    }
